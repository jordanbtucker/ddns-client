...
async function detect({family, detectAddressURL, logFile}) {
  const address = await detectAddress({family, detectAddressURL})
  console.log(address)
  if (logFile !== undefined) {
    const timestamp = new Date().toISOString()
    const message = `detect: ${address}`
    const entry = `${timestamp} ${message}${EOL}`
    await appendFile(logFile, entry)
  }
}

async function resolve(fqdn, {family, dnsServers, logFile}) {
  ...
  if (logFile !== undefined) {
    const timestamp = new Date().toISOString()
    const message = `resolve ${fqdn}: ${addresses.join(',')}`
    const entry = `${timestamp} ${message}${EOL}`
    await appendFile(logFile, entry)
  }
}
...
async function update(
  ...
  if (logFile !== undefined) {
    const timestamp = new Date().toISOString()
    const message = `update ${fqdn}: ${response || 'noupdate'}`
    const entry = `${timestamp} ${message}${EOL}`
    await appendFile(logFile, entry)
  }
}
...
${pkg.name} update <fqdn> <options>
  ...
  Options:
    ...
    --log-file, -L  A path for logging results.
  Output:
